...
const PokemonList = () => {
	...
	const loadPokemonList = async () => {
		const typesQuery = selectedTypes
			.map((selectedTypes) => `types[]=${encodeURIComponent(selectedTypes)}`)
			.join('&');
		const response = await fetch(
			`http://localhost:8000/api/pokemon/search?${typesQuery}&searchQuery=${searchQuery}`
		);
		...
		<div>
			<div id='filters'>
				<div id='searchAndLang'>
					...
					/>
					<select onChange={handleLanguageChange} className='btn btn-primary'>
			...
			<div id='pokemonDetails'>
				{pokemonDetails && pokemonDetails.name ? (
					<div className='position-absolute bg-light rounded-4 p-4 shadow-lg'>
						<input type='button' onClick={() => closeDetails()} className='btn-close' />
			...
			<div id='pokemonList'>
				{pokemonList.length === 0 ? (
					...
					pokemonList.map((pokemon) => (
						<div
							key={pokemon.id}
							onClick={() => pokemonHovered(pokemon.id)}
							className='w-25 text-center p-2 border'
						>
							<h3>{pokemon.name}</h3>
							<img
